...
$sql = "SELECT sv.mssv, sv.hoten, sv.email, sv.khoa, sv.ngaysinh, sv.diem_tb, sv.trang_thai, sv.lop_id, l.tenlop, sv.gioitinh, sv.diachi, sv.sodienthoai
        FROM sinhvien sv
           ...
           <div class="relative" x-data="{ open: false }">
               ...
               <div id="userMenu" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 text-sm text-gray-700 z-10">
                   <div class="flex items-center space-x-3 px-4 py-3 border-b border-gray-200">
                       <div>
                           <div class="text-black font-semibold text-sm"><?= isset($_SESSION['user_name']) ? htmlspecialchars($_SESSION['user_name']) : 'Admin' ?></div>
                           <div class="text-xs leading-4"><?= isset($_SESSION['email']) ? htmlspecialchars($_SESSION['email']) : '' ?></div>
           ...
           <table class="w-full text-left text-gray-700 text-xs border-separate border-spacing-y-2">
               <thead>
                   <tr>
                       ...
                       <th class="font-semibold">Giới tính</th>
                       <th class="font-semibold">Địa chỉ</th>
               ...
               <tbody>
                   <?php if ($result->num_rows > 0): ?>
                       <?php while ($row = $result->fetch_assoc()): ?>
                           <tr class="bg-white rounded-lg shadow-sm">
                               ...
                               <td class="py-3"><?= htmlspecialchars($row['gioitinh']) ?></td>
                               <td class="py-3"><?= htmlspecialchars($row['diachi']) ?></td>
                   ...
                   <?php else: ?>
                       <tr>
                           <td colspan="11" class="text-center py-6 text-gray-500">Chưa có dữ liệu sinh viên.</td>
                       </tr>
                   <?php endif; ?>
               </tbody>
           </table>
